...
function generateLISP(cleanData, xCol, yCol, ruleGroups, fallback, attrMaps, useUTM, fnName) {
  const lispRows = [];
...
(defun ${fnName} ( / data row blk pt tag-val-list ent)
  (setq data (list
${lispRows.join("\n")}
  ))
  (foreach row data
...
function Plotter({
  ...
  showDeleteCols, showDedup, showRemapOnly,
  fnName, downloadName,
}) {
  ...
  const addAttr    = bn => setAttrMaps(m => ({ ...m, [bn]: [...(m[bn] || []), newAttr()] }));
  const removeAttr = (bn, ai) => setAttrMaps(m => ({ ...m, [bn]: m[bn].filter((_, i) => i !== ai) }));
  const updateAttr = (bn, ai, field, val) => setAttrMaps(m => ({ ...m, [bn]: m[bn].map((a, i) => i === ai ? { ...a, [field]: val, ...(field === "col" ? { transforms: {} } : {}) } : a) }));
  const updateAttrTransforms = (bn, ai, transforms) => setAttrMaps(m => ({ ...m, [bn]: m[bn].map((a, i) => i === ai ? { ...a, transforms } : a) }));

  const generate = () => {
    const result = generateLISP(cleanData, xCol, yCol, ruleGroups, fallback, syncedAttrMaps, useUTM, fnName);
